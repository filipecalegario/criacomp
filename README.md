...
Quatro skills de ideação e crítica para os grupos usarem durante o Projeto Autoral: `abrir-o-leque` (gera possibilidades), `afiar-o-eixo` (entrevista até o eixo ficar específico), `derrubar-a-ideia` (advogado do diabo) e `escutar-a-reuniao` (transforma a gravação de uma reunião do grupo em material de trabalho).

São Markdown puro, sem dependência de agente específico. Elas não fazem o projeto de vocês: elas fazem vocês decidirem. Melhoria aceita por pull request conta como Experimento no Portfólio.
...
- Entregas práticas, registro como método e feedback contínuo sobre os projetos
...
O que se avalia é o **registro**, não o artefato, em dois eixos. **Especificidade**: o relato permite que outra pessoa refaça o caminho? **Risco**: você tentou algo que podia não funcionar e registrou o que aconteceu quando não funcionou? Decorre daí que uma Peça que falhou com registro específico vale mais que uma Peça que funcionou com registro genérico.
...
As skills de ideação e crítica desta etapa estão em [`skills/`](skills/).
...
> **Vocabulário.** **Peça** é o item individual do Portfólio. **Artefato** é o item da coleção do Projeto Autoral. São termos distintos de propósito, para ninguém se confundir na hora da entrega.
...
- Criação nas cinco modalidades: imagem, vídeo, texto, código e áudio e música
...
- Comunicações pelo Google Classroom
- Skills de ideação e crítica do Projeto Autoral: [`skills/`](skills/)
